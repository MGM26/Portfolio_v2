    ...
    <section id="contact" className="section">
        <div className="container lg:grid lg:grid-cols-2 lg:items-stretch">
            <div className="mb-12 lg:mb-0 lg:flex lg:flex-col">
                <h2 className="headline-2 lg:max-w-[22ch] reveal-up">
                    Ready to Collaborate? Let's Connect!
                </h2>
                <p className="text-lg text-zinc-400 mb-8 mt-3 max-w-[50ch] lg:max-w-[30ch] reveal-up">
                    I am always open to new learnings, creative ideas, and opportunities. Feel free to reach out to me.
                </p>
                <div className="flex gap-2 mt-auto items-center">
                    {socialLinks.map(({href,icon},key)=> (
                        <a href={href} key={key} target="_blank" className="w-12 h-12 grid place-items-center ring-inset ring-2 ring-zinc-50/5 rounded-full hover:bg-zinc-50 hover:text-zinc-950 transition-[back-ground-color,color] active:bg-zinc-50/50 active:text-zinc-100 reveal-up">
                            {icon}
                        </a>
                    ))}
                </div>
            </div>
            <form action="https://getform.io/f/agdpypnb" method="POST" className="xl:pl-10 2xl:pl-20">
                    <div className="md:grid md:grid-cols-2 md:gap-2 md:items-center mb-4">
                        <div className="">
                            <label htmlFor="name" className="label reveal-up">
                                Your Good Name
                            </label>
                            <input type="text" id="name" name="name" className="text-field reveal-up" placeholder="Mayank Sharma" autoComplete="name" required />
                        </div>
                        <div className="">
                            <label htmlFor="email" className="label reveal-up">
                                Your Email
                            </label>
                            <input type="email" id="email" name="email" className="text-field reveal-up" placeholder="mayank26091999@gmail.com" autoComplete="email" required />
                        </div>
                    </div>
                    <div className="mb-4">
                    <label htmlFor="message" className="label reveal-up">
                            ...
                            <textarea name="message" id="message" className="text-field resize-y min-h-32 max-h-80 reveal-up" placeholder="Hey Mayank, I wish to discuss some prospects with you, that you won't be able to deny. Let's connect for it?" />   
                    </div>
